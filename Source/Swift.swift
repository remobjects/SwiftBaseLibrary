...
protocol ArrayType : ExtensibleCollection, MutableSliceable, ArrayLiteralConvertible {
	...
	//TODO init(count: Int, repeatedValue: Self.GeneratorType.Element)
	...
	/// true if and only if the Array is empty
	var isEmpty: Bool { get }
	//TODOsubscript (index: Int) -> Self.GeneratorType.Element { get set }
	...
	/// Append newElement to the Array in O(1) (amortized)
	//TODO func append(newElement: Self.GeneratorType.Element)

	/// Append elements from `sequence` to the Array
	//TODO func extend<S : Sequence where `Self`.GeneratorType.Element == Self.GeneratorType.Element>(sequence: S)

	/// Remove an element from the end of the Array in O(1).  Returns:
	/// the removed element. Requires: count > 0
	//TODO func removeLast() -> Self.GeneratorType.Element

	/// Insert an element at the given index in O(N).  Requires: atIndex
	/// <= count
	//TODO func insert(newElement: Self.GeneratorType.Element, atIndex: Int)

	/// Remove the element at the given index.  Returns: the removed
	/// element.  Worst case complexity: O(N).  Requires: count > index
	//TODO func removeAtIndex(index: Int) -> Self.GeneratorType.Element
...
protocol BitwiseOperations {
	...
	///   x & ~allZeros == x
	///
	//TODO class var allZeros: Self { get }
}

//TODO
/*
/ *struct* / extension Bool {

	/// Default-initialize Boolean value to `false`.
	init()
	static let false: Bool { 0 as Bool }
	static let true: Bool { 1 as Bool }
}
...
protocol FloatingPointNumber {
	...
	//TODO class var infinity: Self { get }

	/// A quiet NaN.
	//TODO class var NaN: Self { get }
...
protocol Reflectable {
	...
	// RemObjects Silver: we wont implement Reflectable, as Reflection is platform specific
}
...
protocol Sequence {
	//TODO typealias GeneratorType : Generator
	//TODO func generate() -> GeneratorType
}
...
protocol StringElement {
	//TODO class func toUTF16CodeUnit(_: Self) -> CodeUnit
	//TODO class func fromUTF16CodeUnit(utf16: CodeUnit) -> Self
}

protocol StringInterpolationConvertible {
	//TODO class func convertFromStringInterpolation(strings: Self...) -> Self
	//TODO class func convertFromStringInterpolationSegment<T>(expr: T) -> Self
}
...
protocol UnicodeCodec {
	//TODO typealias CodeUnit
	//TODO class func decode<G : Generator where `Self`.CodeUnit == CodeUnit>(inout next: G) -> UnicodeScalar?
	//TODO class func encode<S : Sink where `Self`.CodeUnit == CodeUnit>(input: UnicodeScalar, inout output: S)
}
